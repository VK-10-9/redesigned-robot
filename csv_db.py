...
def explorer_enrollment(
    ...
    """Get enrollment records with pagination and caching"""
    # Create cache key for this query
    cache_key = f"explorer_{state or 'all'}_{district or 'all'}_{date_from or 'all'}_{date_to or 'all'}"
...
def get_demographic_distribution() -> Dict[str, Any]:
    """Return demographic aggregates with caching"""
    key = "demographic_distribution"
    cached = _get_cached(key, CACHE_TTL_LONG)
    if cached is not None:
        return cached

    totals = {"0-5": 0, "5-17": 0, "18+": 0}
    for f in _iter_csv_files(ENROLL_FOLDER):
        with open(f, "r", encoding="utf-8", errors="replace") as fh:
            reader = csv.DictReader(fh)
            for row in reader:
                totals["0-5"] += safe_int(row.get("age_0_5"))
                totals["5-17"] += safe_int(row.get("age_5_17"))
                totals["18+"] += safe_int(row.get("age_18_greater"))

    by_age_group = [
        {"age_group": "0-5", "count": totals["0-5"]},
        {"age_group": "5-17", "count": totals["5-17"]},
        {"age_group": "18+", "count": totals["18+"]},
    ]

    # by_location not available in current CSVs; return empty
    by_location = []
    result = {"by_age_group": by_age_group, "by_location": by_location}
...
def get_unified_state_metrics(limit: int = 100) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        basic_states = csv_get_state_distribution(limit=limit)
